...
def DrawRectOnImage(im, top_left, bot_right):
    color = (0, 255, 0)
    thickness = 2
...
def main():
    ...
    for impath in os.listdir(im_dir_name):
        print(impath)
    ...
    for row in tqdm(r_range):
        ...
        # for col in range(likelyhood_im_shape[1]):
            patch = im[row:row+bb_height, col:col+bb_width]
    ...
    print(r_range_list[0])
    print(r_range_list[1])
    print(c_range_list[0])
    print(c_range_list[1])
    plt.imshow(likelyhood_im)
    plt.show()
    ...
    print(coords.shape)
    print(coords[0])
    # print(coords.reshape((coords.shape[0]*coords.shape[1]), 2))
    ...
    print(new_mean)
    im_with_track = cv2.circle(im, (new_mean[1], new_mean[0]), 1, [255, 0, 0], 2)

    old_center = old_center.astype(int)
    im_with_track2 = cv2.circle(im_with_track, (old_center[0], old_center[1]), 1, [0, 0, 255], 2)
    plt.imshow(im_with_track2)
    plt.show()
